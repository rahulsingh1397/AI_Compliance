...
def load_hr_data(filepath, sample_size=None, random_state=42):
    """
    Load a sample of the HR dataset in a memory-efficient way.
    
    Args:
        filepath: Path to the CSV file
        sample_size: Optional number of records to load
        random_state: Ignored, but kept for compatibility.
    ...
    try:
        # To avoid MemoryError with large datasets, we only read `sample_size` rows.
        # This is not a random sample, but is sufficient for integration testing.
        if sample_size:
            logger.info(f"Loading a sample of {sample_size} records to conserve memory.")
            df = pd.read_csv(filepath, nrows=sample_size)
        else:
            # If no sample size, load the whole file (may cause memory issues)
